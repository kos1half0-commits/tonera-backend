...
export async function runMigrations() {
  ...
  await pool.query(`INSERT INTO settings (key,value) VALUES ('partnership_default_post', E'🔥 TonEra — твой путь к заработку TON!\\n\\n💎 Что такое TonEra?\\nTelegram Mini App где каждый зарабатывает криптовалюту TON прямо с телефона. Без сложностей — заходи и начинай!\\n\\n📊 Как заработать:\\n\\n💰 Стейкинг — вложи от 0.5 TON и получай до 1% дохода в день. Автоматический реинвест увеличивает прибыль!\\n\\n⛏ Майнинг — запусти облачный майнинг и получай TON каждый час в автоматическом режиме\\n\\n🎰 Игры — крути колесо фортуны, играй в слоты и выигрывай реальные TON\\n\\n✅ Задания — подписывайся на каналы, выполняй простые задания и получай мгновенные выплаты\\n\\n📺 Реклама — смотри короткие видео и зарабатывай TON за каждый просмотр\\n\\n👥 Рефералы — приглашай друзей и получай % от их заработка навсегда\\n\\n🏆 Ежедневные бонусы — заходи каждый день и получай награды\\n{PROMO}\\n\\n⚡ Начни зарабатывать прямо сейчас:\\n👉 {REF_LINK}') ON CONFLICT (key) DO UPDATE SET value=EXCLUDED.value`)
  ...
  // === Monetag Ads ===
  await pool.query("INSERT INTO settings (key,value) VALUES ('monetag_enabled','1') ON CONFLICT (key) DO NOTHING")
  await pool.query("INSERT INTO settings (key,value) VALUES ('monetag_zone_id','') ON CONFLICT (key) DO NOTHING")
  ...
  // === OnClickA Ads ===
  await pool.query("INSERT INTO settings (key,value) VALUES ('onclicka_enabled','1') ON CONFLICT (key) DO NOTHING")
